...
class NotesServiceController {
  ...
  getNote(idKey): Note {
    return this.notes.find(note => note.idKey === idKey);
  }
  createNote(title): void {
    // Create a unique idKey that is one larger than the current largest idKey
    let idKey = Math.max(...this.notes.map(note => parseInt(note.idKey)), 0) + 1;
    this.notes.push({
